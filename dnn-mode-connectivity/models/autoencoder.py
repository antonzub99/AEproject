...
from models.AE_init_weights import initialize_weights_normalCurve, initialize_weights_kaimnormCurve, initialize_weights_kaimuniCurve
...
class BasicDecBlockCurve(nn.Module):
    def __init__(self, input_dim, output_dim, fix_points, nonlinearity=nn.LeakyReLU(0.2)):
        ...
        self.conv1 = nn.Conv2d(input_dim, output_dim, kernel_size=3, stride=1,
...
class EncoderCurve(nn.Module):
    def __init__(self, in_channels, input_dim,
        ...
        super().__init__()
        self.conv1 = curves.Conv2d(in_channels, input_dim, fix_points, kernel_size=1,
        ...
        self.layer = self._make_layer(block, num_blocks, fix_points)

        self.conv2 = curves.Conv2d(hidden_dim, latent_dim, kernel_size=1,
                                 stride=1, padding=0, bias=False, fix_points=fix_points)
    ...
    def _make_layer(self, block, num_blocks, fix_points):
        ...
        for _ in range(num_blocks):
            layers.append(block(self.hidden_dim, hidden_dim * 2, fix_points))
            self.hidden_dim *= 2
            self.spatial_size //= 2
...
class DecoderCurve(nn.Module):
    def __init__(self, out_channels, input_dim, hidden_dim,
        ...
        self.conv1 = curves.Conv2d(latent_dim, hidden_dim, kernel_size=1,
                             stride=1, padding=0, bias=False, fix_points=fix_points)

        self.hidden_dim = hidden_dim
        self.layer = self._make_layer(block, num_blocks, fix_points)
        self.conv2 = curves.Conv2d(hidden_dim, hidden_dim, kernel_size=1,
                                 stride=1, padding=0, bias=False, fix_points=fix_points)
        self.nonlinearity = nn.Tanh()
        self.conv3 = curves.Conv2d(hidden_dim, out_channels, kernel_size=1,
                                 stride=1, padding=0, bias=False, fix_points=fix_points)
...
class AEBase(nn.Module):
    def __init__(self, in_channels, input_dim, num_classes,
