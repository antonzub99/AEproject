...
class Linear(CurveModule):
    ...
    def forward(self, inp, coeffs_t):
        weight_t, bias_t = self.compute_weights_t(coeffs_t)
        return F.linear(inp, weight_t, bias_t)
...
class Conv2d(CurveModule):
    ...
    def forward(self, inp, coeffs_t):
        weight_t, bias_t = self.compute_weights_t(coeffs_t)
        return F.conv2d(inp, weight_t, bias_t, self.stride,
...
class _BatchNorm(CurveModule):
    ...
    def forward(self, inp, coeffs_t):
        ...
        weight_t, bias_t = self.compute_weights_t(coeffs_t)
        return F.batch_norm(
            inp, self.running_mean, self.running_var, weight_t, bias_t,
...
class CurveNet(Module):
    ...
    def forward(self, inp, t=None):
        if t is None:
            t = inp.data.new(1).uniform_()
        coeffs_t = self.coeff_layer(t)
        output = self.net(inp, coeffs_t)
        self._compute_l2()
        return output
